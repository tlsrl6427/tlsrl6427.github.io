...
### 해킹으로 Job이 하나 늘어난 Jenkins
  ...
  매일 당일 날짜에 해당하는 데이터가 잘 들어왔는지 DB 툴로 확인해보는데 유기시켜놓고 며칠 다른 공부를 하다가 어느날 보니 최근 데이터가 없는 것이었다! 저번에 API 스펙이 변경됐는데 모르고 업데이트를 안한 경우가 있어 며칠짜리 날렸구나 하면서 들어가봤는데...
...
```
 ...
 3. k() 함수를 통해 소프트웨어를 실행하는 것이다
   ...
   그렇다... 이 녀석은 내 서버를 채굴장으로 쓰려고 했던 것이다. 이 스크립트를 crontab(* * * * *)로 설정하여 매 분마다 실행하였다. 하지만 다행히도? 제대로 실행되지는 않은 것 같다.
...
그래서? 결국? 피해는? 보지않았다? 몇일치의 데이터정도...? 나름 평화롭게 마무리됐다.
